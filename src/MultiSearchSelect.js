  ...
  render() {
    return (
      <div ref={this.setWrapperRef} className={this.props.className} style={{ width: this.props.width }}>
        <div>
          <div className="input-area" contentEditable={false} onFocus={this.props.searchable ? this.show : this.none} onClick={this.props.searchable ? this.none : this.show} style={{ background: 'rgb(34, 34, 34' }}>
            {this.props.showTags && <div contentEditable={false} style={{ display: "inline" }}>
              {this.state.selected.map((e, i) => {
                return (
                  <div className="tags" key={i} style={{ }}>
                    {e}
                    <span
                      onClick={e => {
                        e.preventDefault();
          ...
          {this.state.showOptions && this.props.options && (
            <div className="option-area" style={{ background: this.props.primaryColor, maxHeight: this.props.optionsContainerHeight }}>
...
MultiSearchSelect.defaultProps = {
  ...
  searchPlaceholder: "Search/Compare...",
  selected: [],
  className: "",
  width: "200px",
  primaryColor: "#e1e1e1",
